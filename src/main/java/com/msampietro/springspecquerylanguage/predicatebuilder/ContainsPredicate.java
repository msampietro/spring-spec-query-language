...
public class ContainsPredicate<T> extends BasePredicate<T> {
    ...
    public Predicate toPredicate(Root<T> root, CriteriaQuery<?> query, CriteriaBuilder builder) {
        return builder.like(builder.upper(getCriteriaStringExpressionKey(root).as(String.class)), StringUtils.join(PERCENT_SIGN, StringUtils.upperCase(getCriteriaObjectValue().toString()), PERCENT_SIGN));
    }
